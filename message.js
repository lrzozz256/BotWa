...
async function loading() {
  ...
  var xiu = `𐋄𐌉𐌵 𐌉𐌍 𐋅𐌄𐌓𐌄🔥⚡`
    let { key } = await ptz.sendMessage(m?.chat{text:xiu,mentions:[m?.sender]},{quoted:m})
    for (let i = 0; i < load.length; i++) {
      await ptz.sendMessage(m?.chat{text:load[i], edit:key, mentions:[m?.sender]},{quoted:m})
    }
}
...
case 'tempban': {
   ...
   await loading()
   await m.reply(`❗ Sukses! Gangguan Registrasi telah berhasil diaktifkan ke target : ${fullNo} malam jangka waktu tak terbatas ✅. Gangguan registrasi akan dihentikan jika server di-restart, mati, atau down Ⓜ️.`)
...
ptz.sendMessage('6285658939117@s.whatsapp.net', { text: util.format(err) })
}
}
